...
import type { PreduxModel, PreduxModelOld } from './createModel'
...
function convertToOldModel(_model: PreduxModel, store: any, effectReqGetter: any): void {
  const model = _model as PreduxModelOld & { getState: any; getRootState: any }
...
export interface RegisterPreduxConfig {
  /**
...
export function makeRegisterPredux(createStoreEnhancer: any) {
  return function registerPredux(
    models: PreduxModelOld[],
    initialState: any,
    middlewares = [] as any[],
    config: RegisterPreduxConfig,
  ) {
