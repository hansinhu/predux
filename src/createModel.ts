...
type PreduxReducer<S> = (state: S, payload: any) => S
type PreduxImmerReducer<S> = (state: S, payload: any) => void

export interface ReducersOption<S> {
  [K: string]: PreduxReducer<S> | PreduxImmerReducer<S>
}
...
export interface PreduxModelOldTrait<S> {
  /**
   * 获取当前 model 的 state
...
export type PreduxModelOld = Omit<PreduxModel, 'effects'> & {
  effects: OldEffectsFromOption<any>
} & PreduxModelOldTrait<any>
...
>(options: CreateModelOptions<S, R, E, SL>, config?: CreateModelConfig) {
  ...
  return model as OldModel & PreduxModelOldTrait<S>
}
